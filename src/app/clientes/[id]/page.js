...
export default function FichaCliente({ params }) {
  ...
  const [avisos, setAvisos] = useState([]) // <-- Nuevo estado para el diario
  const [cargando, setCargando] = useState(true)

  // Estados para el formulario del diario
  const [mostrarFormAviso, setMostrarFormAviso] = useState(false)
  const [nuevaFechaAviso, setNuevaFechaAviso] = useState('')
  const [nuevoTextoAviso, setNuevoTextoAviso] = useState('')
  ...
  const cargarDatos = async () => {
    ...
    if (clienteData) {
      ...
      // Cargar los avisos del diario vinculados a este cliente
  ...
  // Función para guardar el WhatsApp/Aviso en el diario
  const guardarAviso = async () => {
    if (!nuevaFechaAviso || !nuevoTextoAviso) {
      alert("Por favor, pon una fecha y un texto (ej: Llega a las 10:00)")
      return
    }

    const { error } = await supabase.from('diario').insert([
      { 
        fecha: nuevaFechaAviso, 
        cliente: cliente.nombre, 
        dni_cliente: cliente.dni, 
        notas: nuevoTextoAviso 
      }
    ])
    ...
    if (error) {
      ...
      setNuevaFechaAviso('')
      setNuevoTextoAviso('')
      cargarDatos() // Recargamos para que aparezca al instante
    }
  }
    ...
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen text-gray-800">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* COLUMNA 2: EXPEDIENTE, DIARIO Y VISITAS */}
          <div className="lg:col-span-8 space-y-6 no-print">
            
            {/* NUEVA SECCIÓN: AVISOS DEL DIARIO */}
            <div className="bg-orange-50 p-8 rounded-3xl shadow-sm border border-orange-100">
              ...
              {mostrarFormAviso && (
                <div className="bg-white p-4 rounded-2xl shadow-inner border border-orange-100 mb-6 flex flex-col md:flex-row gap-3">
                  <input 
                    type="date" 
                    value={nuevaFechaAviso}
                    onChange={(e) => setNuevaFechaAviso(e.target.value)}
                    className="p-3 border rounded-xl text-sm font-bold w-full md:w-auto"
                  />
                  <input 
                    type="text" 
                    placeholder="Ej: Mensaje de WhatsApp: Viene a por el coche a las 11:00..."
                    value={nuevoTextoAviso}
                    onChange={(e) => setNuevoTextoAviso(e.target.value)}
                    className="p-3 border rounded-xl text-sm w-full"
                  />
                  <button 
                    onClick={guardarAviso}
                    className="bg-black text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-800 transition-all"
                  >
                    Guardar
                  </button>
                </div>
              ...
              <div className="space-y-3">
                {avisos.length === 0 ? (
                  ...
                  avisos.map((aviso) => (
                    <div key={aviso.id} className="p-4 bg-white border border-orange-200 rounded-2xl flex flex-col md:flex-row md:items-center gap-4">
                      <span className="bg-orange-100 text-orange-800 font-black text-[10px] px-3 py-1 rounded-lg uppercase tracking-widest whitespace-nowrap">
                        {aviso.fecha}
                      </span>
                      <p className="text-sm font-bold text-gray-700">{aviso.notas}</p>
                    </div>
                  ))
                )}
              </div>
